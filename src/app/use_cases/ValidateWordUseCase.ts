...
export class ValidateWordUseCase {
  ...
  private normalize(w: string): string {
    return (w ?? "").trim().toLocaleLowerCase("fr").normalize("NFC");
  }

  private isEligible(w: string): boolean {
    // Lettres FR + accents + ligatures + tiret/apostrophe, au moins 2 chars
    return /^[a-zA-ZÀ-ÖØ-öø-ÿœŒ'’-]{2,}$/u.test(w);
  }
}
